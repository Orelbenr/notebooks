...
def render_dashboard(
    ...
    ax_res.set_title("HMM Frequency Line Tracking", fontsize=14, fontweight="bold")
    ax_res.set_xlabel("Time Block (k)", fontsize=12)
    ax_res.set_ylabel("Frequency Bin (m)", fontsize=12)
    ax_res.legend(loc="upper right")
    plt.tight_layout()

    # Interactive Plot
    interactive_plot = mo.mpl.interactive(fig_res)
    ...
    *As noted in the paper, both algorithms perform nearly identically, proving the efficacy of the SNR-free probabilistic integration approach.*
    ...
    dashboard = mo.vstack(
        [controls, mo.md("<br>"), interactive_plot, mo.md(metrics_md)]
    )
